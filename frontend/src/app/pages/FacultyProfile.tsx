...
import { ArrowLeft, User, Mail, Linkedin, BookOpen, ExternalLink, Award } from 'lucide-react';
import { Link, useParams, Navigate } from 'react-router-dom';
import { facultyData, lecturersData } from '@/app/data/facultyData';
            ...
            {member.networks && Object.keys(member.networks).length > 0 && (
              ...
              >
                ...
                <div className="space-y-3">
                  {member.networks.linkedin && (
                    <a
                      href={member.networks.linkedin}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center p-3 rounded-lg hover:bg-slate-50 transition-colors border border-slate-100 group"
                    >
                      <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center mr-3">
                        <Linkedin className="w-5 h-5 text-blue-600" />
                      </div>
                      <div className="flex-1">
                        <div className="font-semibold text-slate-900 group-hover:text-[#4CC9BF]">LinkedIn</div>
                  ...
                  {member.networks.googleScholar && (
                    <a
                      href={member.networks.googleScholar}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center p-3 rounded-lg hover:bg-slate-50 transition-colors border border-slate-100 group"
                    >
                      ...
                      <div className="flex-1">
                        <div className="font-semibold text-slate-900 group-hover:text-[#4CC9BF]">Google Scholar</div>
                        <div className="text-xs text-slate-500">Research Publications</div>
                      </div>
                      <ExternalLink className="w-4 h-4 text-slate-400 group-hover:text-[#4CC9BF]" />
                    </a>
                  )}

                  {member.networks.researchgate && (
                    <a
                      href={member.networks.researchgate}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center p-3 rounded-lg hover:bg-slate-50 transition-colors border border-slate-100 group"
                    >
                      ...
                      <div className="flex-1">
                        <div className="font-semibold text-slate-900 group-hover:text-[#4CC9BF]">ResearchGate</div>
                        <div className="text-xs text-slate-500">Academic Network</div>
                      </div>
                      <ExternalLink className="w-4 h-4 text-slate-400 group-hover:text-[#4CC9BF]" />
                    </a>
                  )}

                  {member.networks.orcid && (
                    <a
                      href={member.networks.orcid}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center p-3 rounded-lg hover:bg-slate-50 transition-colors border border-slate-100 group"
                    >
                      ...
                      <div className="flex-1">
                        <div className="font-semibold text-slate-900 group-hover:text-[#4CC9BF]">ORCID</div>
                        <div className="text-xs text-slate-500">Researcher ID</div>
                      </div>
                      <ExternalLink className="w-4 h-4 text-slate-400 group-hover:text-[#4CC9BF]" />
                    </a>
                  )}
