...
interface FacultyMemberFromAPI {
  id: string;
  name: string;
  position: string;
  email: string;
...
export function Faculty() {
  const [facultyMembers, setFacultyMembers] = useState<FacultyMemberFromAPI[]>([]);
                        ...
                        {member.research_interests && member.research_interests.length > 0 && (
                          <div className="mb-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {member.research_interests.slice(0, 3).map((topic, i) => (
              ...
              >
                <Link to={`/faculty/${member.id}`}>
                  <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl hover:border hover:border-[#4CC9BF] transition-all cursor-pointer h-full">
                    <div className="w-16 h-16 bg-[#4CC9BF]/20 rounded-full mb-4 flex items-center justify-center overflow-hidden">
                      {member.profile_image ? (
                        <img src={member.profile_image} alt={member.name} className="w-full h-full object-cover" />
                      ) : (
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <div className="w-20 h-1 bg-[#4CC9BF] mx-auto rounded-full"></div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 justify-items-center">
            {staff.map((member, idx) => (
              ...
              >
                <Link to={`/faculty/${member.id}`}>
                  <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl hover:border hover:border-[#4CC9BF] transition-all cursor-pointer h-full border border-slate-100">
                    <div className="w-16 h-16 bg-[#4CC9BF]/20 rounded-full mb-4 flex items-center justify-center overflow-hidden">
                      {member.profile_image ? (
                        <img src={member.profile_image} alt={member.name} className="w-full h-full object-cover" />
                      ) : (
